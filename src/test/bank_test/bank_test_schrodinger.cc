...
private:
    ...
    void setConcurrency(const std::string & , const std::string & con)
    {
        concurrency = std::stoi(con);
    }
    ...
    void setAccount(const std::string & , const std::string & acc) {
        account = std::stoi(acc);
    }

    void setRunTime(const std::string & , const std::string & time) {
        run_time = std::stoi(time);
    }

    void setCheckOnly(const std::string & , const std::string & ) {
        check_only = true;
    }

    void setCheckAndExec(const std::string & , const std::string & ) {
        check_exec = true;
    }
...
public:
    ...
    void defineOptions(OptionSet & options) override {
        options.addOption(Option("pd-address","p").repeatable(true).argument("pd_address", false).callback(OptionCallback<BankApp>(this, &BankApp::setPDAddr)));
        options.addOption(Option("concurrency","c").validator(new IntValidator(1, 1000)).argument("concurrency", false).callback(OptionCallback<BankApp>(this, &BankApp::setConcurrency)));
